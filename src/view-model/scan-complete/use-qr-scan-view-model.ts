...
export const useQrScanViewModel = (): ScanCompleteQrScanViewModel => {
  ...
  useEffect(() => {
    ;(async () => {
      const cameraPermission = await permissionService.ensureCameraPermission()
      if (!cameraPermission) {
        Alert.alert('귄한 필요', '카메라 권한이 필요합니다.', [
